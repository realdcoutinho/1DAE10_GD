...
#pragma region OpenGLDrawFunctionality
	...
	void DrawCircle(const Point2f& center, float radius, float lineWidth)
	{
		DrawEllipse(center, radius, radius, lineWidth);
	}
...
#pragma region CollisionFunctionality
	...
	bool IsPointInEllipse(const Ellipsef& Ellipsef, const Point2f& mousePos) {
		int res = int((pow((mousePos.x - Ellipsef.center.x), 2) / pow(Ellipsef.radiusX, 2)) + (pow((mousePos.y - Ellipsef.center.y), 2) / pow(Ellipsef.radiusY, 2)));
		if (res > 1) {
			return false;
		}
		else {
...
#pragma region GeneralUtils
	...
	float GetRandomFloat(float minValue, float maxValue, int precision)
	{
