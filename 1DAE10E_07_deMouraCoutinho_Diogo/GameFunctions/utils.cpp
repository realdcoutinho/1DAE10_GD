...
#pragma region CollisionFunctionality
	float GetDistance(Point2f pointOne, Point2f pointTwo) {

		float distance{ sqrtf(float(pow((pointOne.x - pointTwo.x), 2) + float(pow((pointOne.y - pointTwo.y), 2)))) };

		return distance;
	}

	bool IsPointInCircle(Ellipsef circle, Point2f point) {
	...
	bool IsOverlaping(Ellipsef circle, Ellipsef mouseCircle) {

		float distance = GetDistance(circle.center, mouseCircle.center);

		float difference = circle.radiusX + mouseCircle.radiusX;
	...
	bool IsPointInRect(Rectf rect, Point2f point) {
		...
		if (rect.left <= point.x && point.x <= rect.left + rect.width){
				if (rect.bottom <= point.y && point.y <= rect.bottom + rect.height) {
						return true;
					}
					else
						return false;
	...
	bool IsOverlapingRect(Point2f l1, Point2f l2, Point2f r1, Point2f r2) {
