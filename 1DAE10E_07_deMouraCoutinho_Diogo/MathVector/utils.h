...
#pragma region OpenGLDrawFunctionality
	...
	//DrawVector function
	void DrawVector(Vector2f vector, Point2f point = Point2f{ 0,0 });
	float ConvertDegreesToRadians(float degrees);
	Point2f GetCoordinatesFromRadians(float radius, float radians, Point2f offset = Point2f{ 0,0 });
...
#pragma region VectorFunctionality
	...
	Vector2f Add(Vector2f vectorOne, Vector2f VectorTwo);
	std::string ToString(Vector2f& vector);
	Point2f Transform(Vector2f& vector, Point2f& startPoint);
	Vector2f Subtract(Vector2f& vectorOne, Vector2f& VectorTwo);
